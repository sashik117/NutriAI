// Domain calculator for user goals, BMR/TDEE, macros, and water target.
...
export const macroTargetsByGoal = {
  lose: {
    proteinPerKg: 2.1,
    fatPerKg: 0.9,
  },
...
export function calculateDailyNeeds(gender, weight, height, age, activityLevel, targetWeight) {
  ...
  const adjustedCalories = tdee * (goalAdjustments[goal] || goalAdjustments.maintain);
  ...
  const water = Math.round(safeWeight * 35);
...
export class UserNutritionProfile {
  constructor({ gender, weight, height, age, activityLevel, targetWeight }) {
    this.gender = gender;
    this.weight = weight;
    this.height = height;
    this.age = age;
