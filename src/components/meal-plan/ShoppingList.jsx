...
export default function ShoppingList({ day, dayIndex = 0, meals = [], activeMeals, autoGenerateToken = 0, onGenerateRequest }) {
  ...
  const generate = useCallback(async () => {
    if (!sourceMeals.length) {
      toast.error('Спочатку виберіть страви галочкою');
      return;
    }
    setGenerating(true);
    try {
      ...
      setList(nextList);
      setSaved(false);
      toast.success('Список покупок готовий');
    } finally {
      setGenerating(false);
    }
  }, [sourceMeals]);
  ...
  const saveList = () => {
    ...
    setSaved(true);
    toast.success('Список збережено');
  };
  ...
  const copyToClipboard = () => {
    ...
    toast.success('Список скопійовано');
  };
  ...
  return (
    <div className="rounded-3xl border border-border bg-card p-4">
      <div className="mb-3 flex items-start justify-between gap-3">
        <div>
          <p className="text-sm font-extrabold">Список покупок</p>
          <p className="text-xs text-muted-foreground">
            Вибрано страв: <span className="font-semibold text-foreground">{sourceMeals.length}</span>
          </p>
        </div>
      ...
      {!list ? (
        <Button variant="outline" className="h-12 w-full rounded-xl gap-2" onClick={onGenerateRequest || generate} disabled={generating}>
          ...
          {generating ? 'Складаю список...' : 'Скласти список покупок'}
        </Button>
      ) : (
        <div className="space-y-3">
          <div className="flex gap-2">
            ...
            <Button size="sm" variant="ghost" className="h-9 rounded-xl px-3" onClick={generate} disabled={generating}>
              {generating ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Оновити'}
            </Button>
          </div>
          ...
          {list.categories.map((category) => (
            <div key={category.id} className="rounded-2xl bg-muted/35 p-3">
              <p className="mb-2 text-xs font-extrabold text-muted-foreground">{category.name}</p>
                ...
                {category.items.map((item) => (
                  <div key={item.id} className={`rounded-xl bg-background p-2 ${item.checked ? 'opacity-55' : ''}`}>
                    <div className="flex items-center gap-2">
                      ...
                      <button type="button" className="rounded-full p-2 text-muted-foreground hover:text-destructive" onClick={() => deleteItem(category.id, item.id)} title="Видалити">
