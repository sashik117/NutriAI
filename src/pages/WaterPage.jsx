...
export default function WaterPage() {
  ...
  const addWaterMutation = useMutation({
    ...
    onSuccess: (_, amount) => {
      ...
      toast.success(`+${amount} мл додано`);
    },
    onError: (error) => toast.error(error.message || 'Не вдалося додати воду'),
  });
  ...
  const updateWaterMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['waterLogs', today] });
      toast.success('Запис води оновлено');
      setEditingId(null);
      setEditingAmount('');
    },
    onError: (error) => toast.error(error.message || 'Не вдалося оновити воду'),
  });
  ...
  const deleteWaterMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['waterLogs', today] });
      toast.success('Запис води видалено');
    },
    onError: (error) => toast.error(error.message || 'Не вдалося видалити воду'),
  });
  ...
  const mood =
    progress >= 1
      ? { emoji: '🌺', text: 'Чудово! План по воді виконано.', color: 'text-green-500' }
      : progress >= 0.75
        ? { emoji: '🌸', text: 'Майже! Ще трішки.', color: 'text-primary' }
        : progress >= 0.5
          ? { emoji: '🌿', text: 'Половина пройдена, так тримати.', color: 'text-chart-5' }
          : progress >= 0.25
            ? { emoji: '🌱', text: 'Добрий початок, продовжуй.', color: 'text-secondary-foreground' }
            : { emoji: '🥀', text: 'Рослинка просить води.', color: 'text-muted-foreground' };
  ...
  const saveEditing = () => {
    ...
    if (!Number.isFinite(amount) || amount <= 0) {
      toast.error('Вкажіть коректну кількість мл');
      return;
    }
  ...
  return (
    <div className="space-y-6 pt-6">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
        <h1 className="text-2xl font-extrabold">Трекер води 💧</h1>
      </motion.div>
      ...
      >
        ...
        <div className="mt-4 text-center">
          ...
          <span className="text-lg font-medium text-muted-foreground"> / {goal} мл</span>
        </div>
      </motion.div>

      <div className="space-y-3">
        <p className="text-sm font-semibold text-muted-foreground">Додати воду</p>
        <div className="grid grid-cols-4 gap-2">
          {quickAmounts.map((amount) => (
            ...
            >
              ...
              <span className="text-xs font-bold">{amount} мл</span>
            </Button>
          ))}
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-border bg-card p-4">
        <p className="mb-3 text-xs font-semibold text-muted-foreground">Свій об'єм</p>
        ...
        <Button className="mt-3 w-full rounded-xl" onClick={() => addWaterMutation.mutate(customAmount)} disabled={addWaterMutation.isPending}>
          <Plus className="mr-1 h-4 w-4" /> Додати {customAmount} мл
        </Button>
      </div>
      ...
      {waterLogs.length > 0 && (
        <div>
          <p className="mb-2 text-sm font-semibold text-muted-foreground">Сьогодні</p>
                ...
                {editingId === log.id ? (
                  ...
                  <>
                    <div className="flex items-center gap-2">
                      <Droplets className="h-3.5 w-3.5 text-chart-5" />
                      <span className="font-medium">{log.amount_ml} мл</span>
                    </div>
                    <div className="flex items-center gap-1">
